...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        visual_embeddings = self.visual_projection(features).unsqueeze(1)  # (N, 1, D)
        tgt_mask = torch.tril(torch.ones((T, T), dtype=torch.bool, device=features.device))
        
        scores = self.transformer(caption_embeddings, visual_embeddings, tgt_mask)
