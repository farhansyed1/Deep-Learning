...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        i = torch.arange(max_len, dtype=torch.float).unsqueeze(1)
        div_term = torch.pow(10000, -torch.arange(0, embed_dim, 2, dtype=torch.float) / embed_dim)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        Q = Q.reshape(N, S, H, head_dim).transpose(1, 2) 
        K = K.reshape(N, T, H, head_dim).transpose(1, 2) 
        V = V.reshape(N, T, H, head_dim).transpose(1, 2) 
        ...
        attention_weights = self.attn_drop(torch.softmax(attention_scores, dim=3))
        
        attended_values = torch.matmul(attention_weights, V)
        attended_values = attended_values.transpose(1, 2).reshape(N, S, E)
